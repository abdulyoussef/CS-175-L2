	...
	public static void main(String[] args) {
		...
		while (sc.hasNextDouble()) {
			...
			if (price < 10 || price > 1000)
				...
				System.out.println("You win a discount coupon of " + df.format(discount) + " ("
						+ df.format(discountEight * 100) + "% of your purchase)");
			} else if (price <= 150.00) {
				discount = price * discountTen;
				System.out.println("You win a discount coupon of " + df.format(discount) + " ("
						+ df.format(discountTen * 100) + "% of your purchase)");
			} else if (price <= 210.00) {
				discount = price * discountTwelve;
				System.out.println("You win a discount coupon of " + df.format(discount) + " ("
						+ df.format(discountTwelve * 100) + "% of your purchase)");
			} else {
				discount = price * discountFourteen;
				System.out.println("You win a discount coupon of " + df.format(discount) + " ("
						+ df.format(discountFourteen * 100) + "% of your purchase)");
